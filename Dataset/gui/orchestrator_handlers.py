...
from multi_agent_orchestrator import create_orchestrator, WorkflowStatus
...
def orch_clone_repo(self):
    ...
    messagebox.showinfo("Clone Repository", 
        "Clone functionality - Enter GitHub URL in the field above\n"
...
def orch_load_metrics(self):
    ...
    for metric_name, info in all_metrics.items():
        ...
        # Store metric name in item
        self.orch_metrics_listbox.itemconfig(tk.END, {'tags': metric_name})
...
def _orch_run_workflow_thread(self, user_request: str, repo_path: str, selected_metrics: list):
    ...
    try:
        def progress_callback(msg: str):
            ...
            self.root.after(0, lambda: self.orch_log_message(msg, tag))
        ...
        self.root.after(0, lambda: self._orch_handle_result(result))
        
    except Exception as e:
        error_msg = f"ERROR: {str(e)}"
        self.root.after(0, lambda: self.orch_log_message(error_msg, 'error'))
...
def _orch_handle_result(self, result):
    ...
    if status == WorkflowStatus.SUCCESS:
        ...
        if result.generated_code:
            self.orch_log_message("\nLast generated code:", 'info')
            self.orch_log_message(result.generated_code, 'info')
...
def orch_reset(self):
    ...
    self.orch_progress_var.set("Cycle: 0/5 | Status: Idle")
    
    self.orch_log_message("Orchestrator reset. Ready for new workflow!", 'success')

def orch_save_dataset(self):
    """Save generated dataset"""
    if not self.orch_result or not self.orch_result.dataset:
