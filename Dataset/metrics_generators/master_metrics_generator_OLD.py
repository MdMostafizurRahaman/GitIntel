...
class MasterMetricsGenerator:
    ...
    def _generate_file_metrics(self, file_path: str) -> Dict[str, Any]:
        """Generate ALL 64 metrics for a single file with REAL data"""
        metrics = {
            'file': file_path,
            'metrics': {}
        }
        ...
        try:
            ...
            # ========== SIZE Metrics (4) ==========
            metrics['metrics']['num_files'] = 1  # Single file analysis
            metrics['metrics']['num_classes'] = len(re.findall(r'\bclass\s+\w+', content))
            metrics['metrics']['num_methods'] = len(re.findall(r'\b(?:public|private|protected|static)\s+(?:\w+\s+)+\w+\s*\(', content))
            metrics['metrics']['num_statements'] = len(re.findall(r';', content))  # Simplified statement count
            ...
            # npra = Number of Private Attributes
            # fanin = Number of classes that depend on this class
            # fanout = Number of classes this class depends on
            # noi = Number of Interfaces implemented
            # nop = Number of Packages/Imports
            ...
            # ========== LABEL Metrics (3) ==========
            # These are typically labels for ML, can be derived from defects
            defect_data = DefectDetector.analyze_file(file_path)
            metrics['metrics']['defect_type'] = 'none' if not defect_data.get('has_defect') else 'logic_error'
            ...
            # CK Metrics (6) - Chidamber & Kemerer OO Metrics
            if file_path.endswith('.java'):
                try:
                    ck_wmc_dict = WMCCalculator.calculate_from_file(file_path)
                    ck_wmc = max(ck_wmc_dict.values()) if ck_wmc_dict else 0
                    
                    ck_dit = DITCalculator.calculate_from_file(file_path)
                    ck_noc = NOCCalculator.calculate_from_file(file_path)
                    ck_cbo = CBOCalculator.calculate_from_file(file_path)
                    ck_rfc = RFCCalculator.calculate_from_file(file_path)
                    ck_lcom = LCOMCalculator.calculate_from_file(file_path)
                    
                    metrics['metrics']['wmc'] = ck_wmc
                    metrics['metrics']['dit'] = ck_dit if ck_dit is not None else 0
                    metrics['metrics']['noc'] = ck_noc if ck_noc is not None else 0
                    metrics['metrics']['cbo'] = ck_cbo if ck_cbo is not None else 0
                    metrics['metrics']['rfc'] = ck_rfc if ck_rfc is not None else 0
                    metrics['metrics']['lcom'] = ck_lcom if ck_lcom is not None else 0
                except Exception as e:
                    print(f"[DEBUG] CK Metrics error for {file_path}: {e}")
                    metrics['metrics']['wmc'] = 0
                    metrics['metrics']['dit'] = 0
                    metrics['metrics']['noc'] = 0
                    metrics['metrics']['cbo'] = 0
                    metrics['metrics']['rfc'] = 0
                    metrics['metrics']['lcom'] = 0
            
            # OOP Metrics (additional object-oriented metrics)
            try:
                oop_data = OOPAnalyzer.analyze_file(file_path)
                for key, value in oop_data.items():
                    if key not in metrics['metrics']:  # Don't overwrite existing
                        metrics['metrics'][key] = value
            except Exception as e:
                print(f"[DEBUG] OOP Metrics error: {e}")
            ...
            try:
                ...
                print(f"[DEBUG] Coupling Metrics error: {e}")
            
            # Process Metrics (git-based)
            try:
                ...
                print(f"[DEBUG] Process Metrics error: {e}")
